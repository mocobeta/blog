...
想定通りにTripwire発動はしたけれど，メインエージェントが最初に致命的なことを話し始めてしまい，間に合わなかった。ということが，並列実行だとありえます。それで良い場合もあるでしょうが，

## 入力ガードレールの実装例（ブロッキング実行）

`@input_guardrail`デコレータの`run_in_parallel`引数を`False`にすると，ガードレールエージェントの実行後にメインエージェントが実行されます。
...
こうすると，ガードレールエージェントがTripwireを発動させた場合，メインエージェントはスキップされるので，時遅し，ということはなくなります。
